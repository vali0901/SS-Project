...
func InitPhotoRoutes(db *gorm.DB, ocrClient *gosseract.Client, mux *http.ServeMux) {
	...
	mux.Handle("/photos/reports/fit-by-profession", withAuth(http.HandlerFunc(photoController.GetFitByProfessionReport)))
	mux.Handle("/photos/reports/overdue", withAuth(http.HandlerFunc(photoController.GetOverdueReport)))
	mux.Handle("/photos/reports/monthly-compliance", withAuth(http.HandlerFunc(photoController.GetMonthlyComplianceTrendReport)))
